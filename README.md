


# Olá, eu sou o João Vitor! 👋

## 🚀 Sobre Mim
Sou estudante de **Ciência da Computação na UPE** (Universidade de Pernambuco) e apaixonado por explorar ideias criativas e transformá-las em realidade no mundo digital. Busco constantemente evoluir minhas habilidades técnicas e criar soluções que gerem impacto real.

- 🔭 **Atualmente trabalhando em:** Desenvolvimento de jogos digitais para uso educacional.
- 🌱 **Estudando em paralelo:** Aprimorando minhas habilidades em desenvolvimento web.
- ⚡ **Foco:** Criação de experiências interativas e aplicações web robustas.

---

## 🛠️ Tecnologias e Ferramentas

### 💻 Linguagens de Programação
<p>
  ### 💻 Linguagens de Programação

[![Java](https://shields.io)](https://oracle.com)
[![Python](https://shields.io)](https://python.org)
[![JavaScript](https://shields.io)](https://mozilla.org)
[![GDScript](https://shields.io)](https://godotengine.org)

</p>

### 🧰 Ambiente e Frameworks
- **IDE Principal:** VS Code
- **Ecossistema:** Frameworks modernos para desenvolvimento web e motores para criação de jogos.

---

## 📈 Estatísticas do GitHub
<p align="center">
  <img src="https://vercel.app" alt="Estatísticas do GitHub" height="180M" />
  <img src="https://vercel.app" alt="Linguagens mais usadas" height="180M" />
</p>

---

## 🤝 Conecte-se Comigo

Deseja conversar sobre oportunidades, parcerias ou projetos inovadores? Fique à vontade para me mandar uma mensagem!

[![LinkedIn](https://shields.io)](https://www.linkedin.com/in/joao-vitor-ribeiro-99311434b?utm_source=share_via&utm_content=profile&utm_medium=member_android)

_Mal posso esperar para criarmos algo incrível juntos!_
